...
u_stretch = slow_stretch(u, v, problem.epsilon)
v_stretch = slow_stretch(v, u, problem.epsilon)
u_moved = [Point(u.x + us.x, u.y + us.y) for us in u_stretch]
v_moved = [Point(v.x + vs.x, v.y + vs.y) for vs in v_stretch]
u_moved = [um in u_moved if um in points]
v_moved = [vm in v_moved if vm in points]
ax.scatter([p.x for p in u_moved], [p.y for p in u_moved], s=3)
ax.scatter([p.x for p in v_moved], [p.y for p in v_moved], s=3)